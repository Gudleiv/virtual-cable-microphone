...
What the program is for, how to set it up, and what is inside it. The command
lines themselves are in the [README](../README.md); the requirements this was
built against are in [`gc7-virtual-mic-spec.md`](gc7-virtual-mic-spec.md), and
what has actually been measured is in [`testing-notes.md`](testing-notes.md).
...
[`devices-2026-08-12.md`](devices-2026-08-12.md). It is not on the search path —
settings live in `%APPDATA%\vcmic\config.toml` — so either point at it
explicitly or install it:
...
Settings default to `%APPDATA%\vcmic\config.toml`; see the search order in the
[README](../README.md#running), and `config.example.toml` for the full annotated
list of settings. Nothing has to be written by hand — the tray menu covers the
devices, the volumes and the processing, and writes the file itself.

That location was not the first choice. vcmic originally looked for a config
next to the executable, which on a machine where it was compiled rather than
installed is a build output folder — so `--install-autostart` registered a task
that started, found no settings, and stopped. Settings now default to
`%APPDATA%`, the autostart task always records the resolved absolute path, and
none of it has to exist beforehand. Roaming rather than local because that is the
folder people know how to find; the one genuinely machine-specific thing in there
is the endpoint ids, and a config that roams onto a machine without those devices
comes up in the tray's setup state rather than doing any harm.
...
## Reading the counter report

While running, vcmic logs a counter report every `log.stats_interval_s` seconds:
the fill level of each ring and the drift correction being applied to it, plus
underruns, overruns, resyncs, discontinuities, limiter activity and clipped
samples. A healthy session shows a fill level sitting on
`audio.target_buffer_ms`, a drift figure that settles within the first half
minute and then stays put, and zeros everywhere else. `corrected` is cumulative:
it is the skew that would otherwise have accumulated, and it is expected to grow
steadily.
...
A stream that is down at that moment reads `DOWN, rebuilding` instead of a fill
level. Rebuilds are counted per stream and appear both in the periodic report
and in the exit summary as `rebuilt Nx`.
...
A device missing **at startup** is a different question, and the answer changed
once autostart existed. On its own, failing immediately is the more useful
behaviour: telling a misconfigured id apart from an unplugged one is worth more
at that moment than starting anyway. At log on that distinction is impossible to
draw, because the USB stack is still enumerating — hence
`resilience.startup_wait_s`, a retry window defaulting to a minute. Run by hand
with `startup_wait_s = 0` and the strict behaviour is back. With a tray icon
there is a menu to fix it from, so the wait never expires at all: an endpoint
that turns up ten minutes late is still worth picking up, and the icon is there
